...
class AsyncAgentHistoryPromptTransform(AsyncPromptTransform):
    """
    ...
    def __init__(
        ...
        memory: Optional[AsyncTokenBufferMemory] = None,
    ):
    ...
    async def get_prompt(self) -> list[PromptMessage]:
        prompt_messages = []
        num_system = 0
        ...
        max_token_limit = await self._calculate_rest_token(self.prompt_messages, self.model_config)

        model_type_instance = self.model_config.provider_model_bundle.model_type_instance
        model_type_instance = cast(LargeLanguageModel, model_type_instance)

        curr_message_tokens = await model_type_instance.async_get_num_tokens(
