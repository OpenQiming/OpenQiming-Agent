...
from agent_platform_core.helper.tool_parameter_cache import ToolParameterCache, ToolParameterCacheType, \
    ToolParameterCacheAsync
from agent_platform_core.helper.tool_provider_cache import ToolProviderCredentialsCache, ToolProviderCredentialsCacheType
from agent_platform_core.tools.__base import tool_runtime
...
from agent_platform_core.entities.provider_entities import BasicProviderConfig


class ToolConfigurationManager(BaseModel):
    tenant_id: str
    provider_controller: ToolProviderController | PluginToolProviderController
...
class ToolParameterConfigurationManager:
    ...
    def __init__(
        self, tenant_id: str, tool_runtime: Tool, provider_name: str, provider_type: str, identity_id: str
    ) -> None:
    ...
    def _merge_parameters(self) -> list[ToolParameter]:
        ...
        """
        # get tool parameters
        if hasattr(self.tool_runtime, "entity") and hasattr(self.tool_runtime.entity, "parameters"):
    ...
    async def decrypt_tool_parameters_async(self, parameters: dict[str, Any]) -> dict[str, Any]:
        """
        decrypt tool parameters with tenant id

        return a deep copy of parameters with decrypted values
        """
        if hasattr(self.tool_runtime, "entity") and hasattr(self.tool_runtime.entity, "identity"):
            tool_name = self.tool_runtime.entity.identity.name
        else:
            tool_name = self.tool_runtime.identity.name

        cache = ToolParameterCacheAsync(
            tenant_id=self.tenant_id,
            provider=f"{self.provider_type}.{self.provider_name}",
            tool_name=tool_name,
            cache_type=ToolParameterCacheType.PARAMETER,
        ...
        cached_parameters = await cache.get()
        if cached_parameters:
            return cached_parameters
