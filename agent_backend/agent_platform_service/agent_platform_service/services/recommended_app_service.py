...
from agent_platform_common.constants.languages import languages
from agent_platform_core.models.db_model.model import App, RecommendedApp
...
class RecommendedAppService:
    ...
    def get_recommended_apps_and_categories(cls, language: str) -> dict:
        ...
        if mode == 'remote':
            try:
                result = cls._fetch_recommended_apps_from_agent_platform_official(language)
            except Exception as e:
                logger.warning(f'fetch recommended apps from agent platform official failed: {e}, switch to built-in.')
    ...
    @classmethod
    def _fetch_recommended_apps_from_agent_platform_official(cls, language: str) -> dict:
        """
        Fetch recommended apps from agent platform official.
        :param language: language
        :return:
        """
        domain = current_app.config.get('HOSTED_FETCH_APP_TEMPLATES_REMOTE_DOMAIN', None)
    ...
    def get_recommend_app_detail(cls, app_id: str) -> Optional[dict]:
        ...
        if mode == 'remote':
            try:
                result = cls._fetch_recommended_app_detail_from_agent_platform_official(app_id)
            except Exception as e:
                logger.warning(
                    f'fetch recommended app detail from agent platform official failed: {e}, switch to built-in.')
    ...
    @classmethod
    def _fetch_recommended_app_detail_from_agent_platform_official(cls, app_id: str) -> Optional[dict]:
        """
        Fetch recommended app detail from agent platform official.
        :param app_id: App ID
        :return:
        """
        domain = current_app.config.get('HOSTED_FETCH_APP_TEMPLATES_REMOTE_DOMAIN', None)
    ...
    def fetch_all_recommended_apps_and_export_datas(cls):
        ...
        for language in languages:
            try:
                ...
                logger.warning(f'fetch recommended apps from Agent Platform official failed: {e}, skip.')
                continue
            ...
            for recommended_app in result.get('recommended_apps'):
                ...
                # get app detail
                app_detail = cls._fetch_recommended_app_detail_from_agent_platform_official(app_id)
                if not app_detail:
                    continue
