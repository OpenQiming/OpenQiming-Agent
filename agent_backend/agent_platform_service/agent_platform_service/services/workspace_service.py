...
from agent_platform_basic.models.db_model import Tenant, TenantAccountJoin
...
class WorkspaceService:
    ...
    def get_tenant_info(cls, tenant: Tenant):
        ...
        if can_replace_logo and TenantService.has_roles(tenant,
                                                        [TenantAccountRole.OWNER, TenantAccountRole.ADMIN]):
            base_url = current_app.config.get('FILES_URL')
            replace_webapp_logo = f'{base_url}/files/workspaces/{tenant.id}/webapp-logo' if tenant.custom_config_dict.get(
                'replace_webapp_logo') else None
