...
from agent_platform_core.file import file_repository
from agent_platform_core.helper import async_ssrf_proxy, ssrf_proxy
...
from agent_platform_basic.extensions.ext_database import async_db, db
from agent_platform_basic.extensions.ext_storage import async_storage, storage
    ...
    *,
    ...
    match f.type:
        case FileType.IMAGE:
            ...
            if agent_platform_config.MULTIMODAL_SEND_IMAGE_FORMAT == "url":
                data = _to_url(f)
            else:
        ...
        case FileType.VIDEO:
            if agent_platform_config.MULTIMODAL_SEND_VIDEO_FORMAT == "url":
                data = _to_url(f)
            else:
                data = _to_base64_data_string(f)
            if f.extension is None:
...
async def async_download(f: File, /):
    async with async_db.AsyncSessionLocal() as session:
        if f.transfer_method == FileTransferMethod.TOOL_FILE:
            tool_file = await file_repository.async_get_tool_file(session=session, file=f)
            return await _async_download_file_content(tool_file.file_key)
        elif f.transfer_method == FileTransferMethod.LOCAL_FILE:
            upload_file = await file_repository.async_get_upload_file(session=session, file=f)
            return await _async_download_file_content(upload_file.key)
    # remote file
    response = await async_ssrf_proxy.get(f.remote_url, follow_redirects=True)
    response.raise_for_status()
    return response.content
...
async def _async_download_file_content(path: str, /):
    """
    Download and return the contents of a file as bytes.
    ...
    Raises:
        ValueError: If the loaded file is not a bytes object.
    """
    data = await async_storage.load(path, stream=False)
    if not isinstance(data, bytes):
        raise ValueError(f"file {path} is not a bytes object")
    return data


async def _get_encoded_string(f: File, /):
    async with async_db.AsyncSessionLocal() as session:
        match f.transfer_method:
            ...
            case FileTransferMethod.LOCAL_FILE:
                upload_file = await file_repository.get_upload_file(session=session, file=f)
                data = await _async_download_file_content(upload_file.key)
            case FileTransferMethod.TOOL_FILE:
                tool_file = await file_repository.get_tool_file(session=session, file=f)
                data = await _async_download_file_content(tool_file.file_key)
