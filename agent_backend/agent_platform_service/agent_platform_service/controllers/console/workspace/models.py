...
from agent_platform_basic.libs import DbUtils, StringUtils
...
from agent_platform_service.fastapi_fields.req.console.default_model_req import DefaultModelReq
from agent_platform_service.services import model_provider_service
...
@console_api.get("/workspaces/current/default-model", status_code=status.HTTP_200_OK)
async def default_model_api_get(model_type: str,tenant_id: Optional[str] = None,
                                model_provider_service: ModelProviderService = Depends(ModelProviderService),
                                current_user: Account = Depends(login_user)):
    # TODO Change Tenant ID FROM FRONTEND

    if StringUtils.is_blank(tenant_id):
        tenant_id = current_user.current_tenant_id

    default_model_entity = await model_provider_service.get_default_mode_of_model_type_async(
        tenant_id=tenant_id,
...
@console_api.post("/workspaces/current/default-model", status_code=status.HTTP_200_OK)
async def default_model_api_post(request_body: DefaultModelReq,
                                 model_provider_service: ModelProviderService = Depends(ModelProviderService),
                                 current_user: Account = Depends(login_user)):
    model_settings = request_body.app_model_settings
    for model_setting in model_settings:
        if 'model_type' not in model_setting.model_fields or model_setting.model_type not in [mt.value for mt in ModelType]:
            raise ValueError('Invalid model_type')

        if 'provider' not in model_setting.model_fields:
            continue

        if 'model' not in model_setting.model_fields:
            raise ValueError('Invalid model')
        # TODO Change Tenant ID FROM FRONTEND
        try:
            tenant_id = current_user.current_tenant_id
            if StringUtils.is_not_blank(request_body.tenant_id):
                tenant_id = request_body.tenant_id
            await model_provider_service.update_default_model_of_model_type_async(
                tenant_id=tenant_id,
...
class ModelProviderModelApi(Resource):
    ...
    def post(self, provider: str):
        ...
        if provider == 'tool_chain':
            model = args['model'] + '_' + args['credentials']['task_id']
...
class TenantModelProviderAvailableModelApi(Resource):

    @setup_required
    @login_required
    @account_initialization_required
    def get(self, model_type, tenant_id):
...
api.add_resource(TenantModelProviderAvailableModelApi, '/workspaces/<tenant_id>/models/model-types/<string:model_type>')
