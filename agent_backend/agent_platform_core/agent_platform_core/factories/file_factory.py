...
from agent_platform_common.constants import AUDIO_EXTENSIONS, DOCUMENT_EXTENSIONS, IMAGE_EXTENSIONS, VIDEO_EXTENSIONS
from agent_platform_core.file import File, FileBelongsTo, FileTransferMethod, FileType, FileUploadConfig
from agent_platform_core.helper import ssrf_proxy, async_ssrf_proxy
from agent_platform_basic.extensions.ext_database import db, async_db
from agent_platform_core.models.db_model.model import MessageFile, UploadFile
...
async def async_build_from_mapping(
        *,
        mapping: Mapping[str, Any],
        tenant_id: str,
        config: FileUploadConfig | None = None,
    ...
    build_functions: dict[FileTransferMethod, Callable] = {
        FileTransferMethod.LOCAL_FILE: _async_build_from_local_file,
        FileTransferMethod.REMOTE_URL: _async_build_from_remote_url,
        FileTransferMethod.TOOL_FILE: _async_build_from_tool_file,
    }
...
async def async_build_from_mappings(
        *,
        mappings: Sequence[Mapping[str, Any]],
        config: FileUploadConfig | None = None,
        tenant_id: str,
    ...
    files = [
        await async_build_from_mapping(
            mapping=mapping,
            tenant_id=tenant_id,
            config=config,
        )
...
async def _async_build_from_local_file(
        *,
        mapping: Mapping[str, Any],
        tenant_id: str,
        transfer_method: FileTransferMethod,
...
async def _async_build_from_remote_url(
        *,
        mapping: Mapping[str, Any],
        tenant_id: str,
        transfer_method: FileTransferMethod,
    ...
    mime_type, filename, file_size = await _async_get_remote_file_info(url)
    extension = mimetypes.guess_extension(mime_type) or "." + filename.split(".")[-1] if "." in filename else ".bin"
...
async def _async_get_remote_file_info(url: str):
    file_size = -1
    filename = url.split("/")[-1].split("?")[0] or "unknown_file"
    mime_type = mimetypes.guess_type(filename)[0] or ""

    resp = await async_ssrf_proxy.head(url, follow_redirects=True)
    resp = cast(httpx.Response, resp)
    if resp.status_code == httpx.codes.OK:
        if content_disposition := resp.headers.get("Content-Disposition"):
            filename = str(content_disposition.split("filename=")[-1].strip('"'))
...
async def _async_build_from_tool_file(
        *,
        mapping: Mapping[str, Any],
        tenant_id: str,
        transfer_method: FileTransferMethod,
    ...
    async with async_db.AsyncSessionLocal() as session:
        rs = await session.execute(select(ToolFile).filter(
            ToolFile.id == mapping.get("tool_file_id"),
            ToolFile.tenant_id == tenant_id,
        ))
        tool_file = rs.scalar_one_or_none()
