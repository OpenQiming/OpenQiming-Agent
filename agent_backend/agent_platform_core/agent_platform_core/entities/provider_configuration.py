...
from agent_platform_core.entities.model_entities import ModelStatus, ModelWithProviderEntity, SimpleModelProviderEntity
from agent_platform_core.entities.provider_entities import (
    CustomConfiguration,
    ModelSettings,
    SystemConfiguration,
...
from agent_platform_core.helper.model_provider_cache import ProviderCredentialsCache, ProviderCredentialsCacheType
from agent_platform_core.model_runtime.entities.model_entities import FetchFrom, ModelType
from agent_platform_core.model_runtime.entities.provider_entities import (
    ConfigurateMethod,
    CredentialFormSchema,
    FormType,
    ProviderEntity,
)
from agent_platform_core.model_runtime.model_providers import model_provider_factory
from agent_platform_core.model_runtime.model_providers.__base.ai_model import AIModel
from agent_platform_core.model_runtime.model_providers.__base.model_provider import ModelProvider
from agent_platform_basic.extensions.ext_database import db
from agent_platform_core.models.db_model.provider import (
...
class ProviderConfiguration(BaseModel):
    ...
    def add_or_update_custom_credentials(self, credentials: dict) -> None:
        ...
        if provider_record:
            ...
            provider_record.updated_at = datetime.datetime.now().replace(tzinfo=None)
            db.session.commit()
        else:
    ...
    def add_or_update_custom_model_credentials(self, model_type: ModelType, model: str, credentials: dict) -> None:
        ...
        if self.provider.provider == 'tool_chain':
        ...
        if provider_model_record:
            ...
            provider_model_record.updated_at = datetime.datetime.now().replace(tzinfo=None)
            db.session.commit()
        else:
    ...
    def delete_model_setting(self, tenant_id: str, provider: str, model: str):
        provider_model_settings = db.session.query(ProviderModelSetting).filter(
            ProviderModelSetting.tenant_id == tenant_id,
            ProviderModelSetting.provider_name == provider,
            ProviderModelSetting.model_name == model).all()

        for provider_model_setting in provider_model_settings:
            db.session.delete(provider_model_setting)
    ...
    def enable_model(self, model_type: ModelType, model: str) -> ProviderModelSetting:
        ...
        if model_setting:
            ...
            model_setting.updated_at = datetime.datetime.now().replace(tzinfo=None)
            db.session.commit()
        else:
