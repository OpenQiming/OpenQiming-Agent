...
from agent_platform_core.tools.entities.api_entities import UserToolProvider
...
class WorkflowToolManageService:
    ...
    async def create_workflow_tool_async(self, user_id: str, tenant_id: str, workflow_app_id: str, tool_param: dict):
        tool_name = tool_param.get('name')
        # check if the name is unique
        existing_workflow_tool_provider = await self.session.execute(select(WorkflowToolProvider).filter(
            WorkflowToolProvider.tenant_id.in_([tenant_id, agent_platform_config.GLOBAL_TENANT_ID]),
            # name or app_id
            or_(WorkflowToolProvider.name == tool_name, WorkflowToolProvider.app_id == workflow_app_id)
        ))
        existing_workflow_tool_provider = existing_workflow_tool_provider.scalar_one_or_none()

        if existing_workflow_tool_provider is not None:
            raise BaseHTTPException(f'Tool with name {tool_name} or app_id {workflow_app_id} already exists')

        app_query = await self.session.execute(select(App).filter(
            App.id == workflow_app_id,
            App.tenant_id == tenant_id
        ))
        app: App = app_query.scalar_one_or_none()

        if app is None:
            raise BaseHTTPException(f'App {workflow_app_id} not found')

        workflow_query = await self.session.execute(select(Workflow).filter(Workflow.id == app.workflow_id))
        workflow = workflow_query.scalar_one_or_none()
        if workflow is None:
            raise BaseHTTPException(f'Workflow not found for app {workflow_app_id}')
        ...
        workflow_tool_provider = WorkflowToolProvider(
            ...
            label=tool_param.get('label'),
            icon=json.dumps(tool_param.get('icon'), ensure_ascii=False),
            description=tool_param.get('description'),
            parameter_configuration=json.dumps(tool_param.get('parameters'), ensure_ascii=False),
            privacy_policy=tool_param.get('privacy_policy'),
            version=workflow.version,
        )

        try:
            await WorkflowToolProviderController.async_from_db(workflow_tool_provider)
        except Exception as e:
            raise ValueError(str(e))

        self.session.add(workflow_tool_provider)
        await self.session.commit()

        return {
            'result': 'success',
            'workflow_tool_id': workflow_tool_provider.id
        }
    ...
    def create_workflow_tool(cls, user_id: str, tenant_id: str, workflow_app_id: str, name: str,
        ...
        workflow_tool_provider = WorkflowToolProvider(
            ...
            parameter_configuration=json.dumps(parameters, ensure_ascii=False),
    ...
    def update_workflow_tool(cls, user_id: str, tenant_id: str, workflow_tool_id: str,
        ...
        workflow_tool_provider.icon = json.dumps(icon, ensure_ascii=False)
        workflow_tool_provider.description = description
        workflow_tool_provider.parameter_configuration = json.dumps(parameters, ensure_ascii=False)
        workflow_tool_provider.privacy_policy = privacy_policy
        workflow_tool_provider.workflow_id = workflow.id
        workflow_tool_provider.version = workflow.version
        workflow_tool_provider.updated_at = datetime.now().replace(tzinfo=None)
    ...
    def list_tenant_workflow_tools(cls, user_id: str, tenant_id: str) -> list[UserToolProvider]:
        ...
        """
        tenant_ids = [tenant_id, agent_platform_config.GLOBAL_TENANT_ID]
        db_tools = db.session.query(WorkflowToolProvider).filter(
            WorkflowToolProvider.tenant_id.in_(tenant_ids),
            WorkflowToolProvider.version != AppStatus.DRAFT.value
        ).all()
        ...
        for tool in tools:
            ...
            ToolTransformService.repack_provider(user_tool_provider, tenant_id)
    ...
    @classmethod
    def get_workflow_tool_by_app_id_and_version(cls, user_id: str, tenant_id: str,
                                                workflow_app_id: str, workflow_version: str = None) -> dict:
        """
        Get a workflow tool.
        :param user_id: the user id
        :param tenant_id: the tenant id
        ...
        if workflow_version:
            db_tool: WorkflowToolProvider = db.session.query(WorkflowToolProvider).filter(
                ...
                WorkflowToolProvider.version == workflow_version,
            ).first()
        else:
            db_tool: WorkflowToolProvider = db.session.query(WorkflowToolProvider).filter(
                WorkflowToolProvider.tenant_id == tenant_id,
                WorkflowToolProvider.app_id == workflow_app_id
            ).order_by(WorkflowToolProvider.version.desc()).first()
        ...
        return {
            ...
            'workflow_tool_id': db_tool.id,
            'workflow_app_id': db_tool.app_id,
            'workflow_id': db_tool.workflow_id,
