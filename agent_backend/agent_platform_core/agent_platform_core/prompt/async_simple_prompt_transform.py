...
from agent_platform_core.app.app_config.entities import PromptTemplateEntity
...
class AsyncSimplePromptTransform(AsyncPromptTransform):
    """
    ...
    async def get_prompt(
        ...
        if model_mode == ModelMode.CHAT:
            prompt_messages, stops = await self._get_chat_model_prompt_messages(
                app_mode=app_mode,
            ...
            prompt_messages, stops = await self._get_completion_model_prompt_messages(
                app_mode=app_mode,
                pre_prompt=prompt_template_entity.simple_prompt_template,
                inputs=inputs,
                query=query,
    ...
    async def _get_chat_model_prompt_messages(
        ...
        if memory:
            prompt_messages = await self._append_chat_histories(
                memory=memory,
