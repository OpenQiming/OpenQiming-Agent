...
from agent_platform_core.model_runtime.entities.model_entities import PriceType
...
from agent_platform_core.model_runtime.model_providers.openai._common import _CommonOpenAI
...
class OpenAITextEmbeddingModel(_CommonOpenAI, TextEmbeddingModel):
    ...
    async def _async_invoke(self, model: str, credentials: dict,
        ...
        credentials_kwargs = self._to_credential_kwargs(credentials)
        # init model client
        client = AsyncOpenAI(**credentials_kwargs)
        ...
        for i in _iter:
            # call embedding model
            embeddings_batch, embedding_used_tokens = await self._async_embedding_invoke(
                model=model,
                client=client,
                texts=tokens[i: i + max_chunks],
                extra_model_kwargs=extra_model_kwargs
    ...
    async def async_validate_credentials(self, model: str, credentials: dict) -> None:
        """
        Validate model credentials
        ...
        try:
            ...
            # call embedding model
            await self._async_embedding_invoke(
                model=model,
                client=client,
                texts=['ping'],
                extra_model_kwargs={}
    ...
    async def _async_embedding_invoke(self, model: str, client: AsyncOpenAI, texts: Union[list[str], str],
                                      extra_model_kwargs: dict) -> tuple[list[list[float]], int]:
        """
        Invoke embedding model
