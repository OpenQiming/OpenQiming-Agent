...
from agent_platform_core.app.entities.queue_entities import QueueMessageReplaceEvent
from agent_platform_core.moderation.base import ModerationAction, ModerationOutputsResult
from agent_platform_core.moderation.factory import ModerationFactory
...
class AsyncOutputModeration(BaseModel):
    tenant_id: str
    app_id: str
    ...
    async def moderation_completion(self, completion: str, public_event: bool = False) -> str:
        ...
        if public_event:
            await self.queue_manager.publish(QueueMessageReplaceEvent(text=final_output), PublishFrom.TASK_PIPELINE)

        return final_output

    def start_thread(self) -> threading.Thread:
        buffer_size = agent_platform_config.MODERATION_BUFFER_SIZE
        thread = threading.Thread(
            target=self.worker,
            kwargs={
                "flask_app": current_app._get_current_object(),
                "buffer_size": buffer_size if buffer_size > 0 else agent_platform_config.MODERATION_BUFFER_SIZE,
            },
        )
