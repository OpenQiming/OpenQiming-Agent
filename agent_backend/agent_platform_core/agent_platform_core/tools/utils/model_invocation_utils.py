...
from agent_platform_core.async_model_manager import AsyncModelManager
...
from agent_platform_core.model_runtime.model_providers.__base.large_language_model import LargeLanguageModel, ModelPropertyKey
...
from agent_platform_core.models.db_model.tools import ToolModelInvoke
...
class ModelInvocationUtils:
    ...
    async def async_invoke(
        ...
        # get model instance
        model_instance = await model_manager.get_default_model_instance(
            tenant_id=tenant_id,
            model_type=ModelType.LLM,
        )

        # get prompt tokens
        prompt_tokens = await model_instance.get_llm_num_tokens(prompt_messages)
        ...
        async with async_db.AsyncSessionLocal() as session:
            ...
            try:
                response: LLMResult = await model_instance.invoke_llm(
                    prompt_messages=prompt_messages,
                    model_parameters=model_parameters,
                    tools=[],
                    stop=[],
