...
from agent_platform_core.app.app_config.entities import ModelConfigEntity
...
class AgentChatAppConfigManager(BaseAppConfigManager):
    ...
    def get_app_config(
        ...
        fronted_model_config: Mapping[str, Any] = None,
    ) -> AgentChatAppConfig:
        """
        ...
        :param is_multi_chat: bool
        :param fronted_model_config: fronted model config
        :return:
        """
        ...
        # 对于多模型调用，从前端传来的参数里取model配置
        else:
            logging.info("start to get model config for multi-chat!")
            completion_params = fronted_model_config.get("completion_params")
            stop = []
            ...
            # get model mode
            model_mode = fronted_model_config.get("mode")

            model_config_to_convert = ModelConfigEntity(
            provider=fronted_model_config["provider"],
            model=fronted_model_config["name"],
            mode=model_mode,
