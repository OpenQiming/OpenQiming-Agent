...
from agent_platform_core.app.app_config.entities import EasyUIBasedAppModelConfigFrom
from agent_platform_core.app.apps.async_base_app_generator import AsyncBaseAppGenerator
from agent_platform_core.app.apps.async_base_app_queue_manager import AsyncAppQueueManager, GenerateTaskStoppedError
...
from agent_platform_core.app.task_pipeline.async_easy_ui_based_generate_task_pipeline import AsyncEasyUIBasedGenerateTaskPipeline
from agent_platform_core.prompt.utils.prompt_template_parser import PromptTemplateParser
...
from agent_platform_core.models.db_model.model import App, AppModelConfig, Conversation, EndUser, Message, MessageFile
...
from agent_platform_basic.exceptions.services.conversation import ConversationCompletedError, ConversationNotExistsError
from agent_platform_service.fields.app_model_config_async import AppModelConfigAsync

logger = logging.getLogger(__name__)


class AsyncMessageBasedAppGenerator(AsyncBaseAppGenerator):
    async def _handle_response(
        ...
        ChatbotAppBlockingResponse,
        CompletionAppBlockingResponse,
        AsyncGenerator[Union[ChatbotAppStreamResponse, CompletionAppStreamResponse], None]
    ]:
        """
        Handle response.
        ...
        generate_task_pipeline = AsyncEasyUIBasedGenerateTaskPipeline(
    ...
    async def _get_conversation_for_multi_agent(
        self, conversation_id: str, user: Union[Account, EndUser]
    ) -> Conversation:
        conversation_filter = [
        ...
        async with async_db.AsyncSessionLocal() as session:
            conversation_scalar = await session.execute(select(Conversation).filter(and_(*conversation_filter)))
            conversation = conversation_scalar.scalar_one_or_none()
    ...
    async def _get_conversation_by_user(
    ...
    async def _get_app_model_config(self, app_model: App, conversation: Optional[Conversation] = None) -> AppModelConfig:
        async with async_db.AsyncSessionLocal() as session:
            if conversation:
                app_model_config_scalar = await session.execute(
                    select(AppModelConfig).filter(AppModelConfig.id == conversation.app_model_config_id,
                                                  AppModelConfig.app_id == app_model.id))
                app_model_config = app_model_config_scalar.scalar_one_or_none()
            ...
            app_model_config = await AppModelConfigAsync(session).get_app_model_config(app_model)
        ...
        async with async_db.AsyncSessionLocal() as session:
            if not conversation:
                conversation = Conversation(
                    ...
                    model_id=model_id,
                    override_model_configs=json.dumps(override_model_configs, ensure_ascii=False) if override_model_configs else None,
                ...
                conversation.updated_at = datetime.now().replace(tzinfo=None)
    ...
    async def _get_conversation(self, conversation_id: str) -> Conversation:
        ...
        """
        async with async_db.AsyncSessionLocal() as session:
            conversation = await session.execute(select(Conversation).filter(Conversation.id == conversation_id))
