...
from agent_platform_core.model_runtime.entities.llm_entities import LLMResult
from agent_platform_core.model_runtime.entities.message_entities import PromptMessage, SystemPromptMessage, UserPromptMessage
from agent_platform_core.tools.entities.tool_entities import ToolProviderType
from agent_platform_core.tools.tool.tool import Tool
from agent_platform_core.tools.utils.model_invocation_utils import ModelInvocationUtils
from agent_platform_core.tools.utils.web_reader_tool import get_url
...
class BuiltinTool(Tool):
    ...
    async def async_invoke_model(self, user_id: str, prompt_messages: list[PromptMessage], stop: list[str]) -> LLMResult:
        """
        invoke model
        ...
        """
        # invoke model
        return await ModelInvocationUtils.async_invoke(
            user_id=user_id,
            tenant_id=self.runtime.tenant_id,
            tool_type="builtin",
            tool_name=self.identity.name,
