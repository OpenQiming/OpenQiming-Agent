...
from agent_platform_core.helper.code_executor.code_executor import CodeExecutor, CodeLanguage
...
class SimpleCode(BuiltinTool):
    ...
    async def _async_invoke(self, user_id: str, tool_parameters: dict[str, Any]) -> ToolInvokeMessage | list[ToolInvokeMessage]:
        """
        invoke simple code
        """
        ...
        result = await CodeExecutor.async_execute_code(language, "", code)
