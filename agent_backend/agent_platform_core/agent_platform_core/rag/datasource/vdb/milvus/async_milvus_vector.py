...
from agent_platform_core.rag.datasource.entity.embedding import Embeddings
from agent_platform_core.rag.datasource.vdb.field import Field
from agent_platform_core.rag.datasource.vdb.vector_base import BaseVector
from agent_platform_core.rag.datasource.vdb.vector_factory import AbstractVectorFactory
...
class AsyncMilvusVector(BaseVector):
    ...
    def create_collection(
        ...
        with redis_client.lock(lock_name, timeout=20):
            ...
            if not utility.has_collection(self._collection_name, using=alias):
                ...
                self._client._create_collection_with_schema(collection_name=collection_name,
...
class MilvusVectorFactory(AbstractVectorFactory):
    def init_vector(self, dataset: Dataset, attributes: list, embeddings: Embeddings) -> AsyncMilvusVector:
        ...
        config = agent_platform_config.MILVUS_HOST
        return AsyncMilvusVector(
            collection_name=collection_name,
            config=MilvusConfig(
                host=agent_platform_config.MILVUS_HOST,
                port=agent_platform_config.MILVUS_PORT,
                user=agent_platform_config.MILVUS_USER,
                password=agent_platform_config.M,
