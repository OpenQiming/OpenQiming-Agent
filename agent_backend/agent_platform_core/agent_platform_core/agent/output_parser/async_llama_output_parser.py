...
class AsyncLlamaAgentOutputParser:
    @classmethod
    async def handle_react_stream_output(cls, llm_response: AsyncGenerator[LLMResultChunk, None], usage_dict: dict) -> \
            AsyncGenerator[Union[str, AgentScratchpadUnit.Action], None]:

        def parse_llama_func_response(response: str):
            # 定义正则表达式，匹配函数名和参数
            function_regex = r"<function=([\w]+)>(.*?)</function>"  # TODO rm '\s' after modifing prompt
