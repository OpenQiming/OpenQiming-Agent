...
def add_document_to_index_task(dataset_document_id: str):
    ...
    try:
        ...
        logging.info(
            click.style('Document added to index: {} latency: {}'.format(dataset_document.id, end_at - start_at),
                        fg='green'))
    except Exception as e:
        logging.exception("add document to index failed")
        dataset_document.enabled = False
        dataset_document.disabled_at = datetime.datetime.now().replace(tzinfo=None)
