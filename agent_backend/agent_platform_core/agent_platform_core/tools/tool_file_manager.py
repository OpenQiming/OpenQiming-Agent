...
from agent_platform_basic.extensions.ext_storage import storage, async_storage
from agent_platform_core.models.db_model.model import MessageFile
...
class ToolFileManager:
    ...
    def sign_file(tool_file_id: str, extension: str) -> str:
        ...
        """
        base_url = agent_platform_config.FILES_URL
        file_preview_url = f"{base_url}/files/tools/{tool_file_id}{extension}"

        timestamp = str(int(time.time()))
        nonce = os.urandom(16).hex()
        data_to_sign = f"file-preview|{tool_file_id}|{timestamp}|{nonce}"
        secret_key = agent_platform_config.SECRET_KEY.encode() if agent_platform_config.SECRET_KEY else b""
    ...
    def verify_file(file_id: str, timestamp: str, nonce: str, sign: str) -> bool:
        ...
        return current_time - int(timestamp) <= agent_platform_config.FILES_ACCESS_TIMEOUT
    ...
    @staticmethod
    async def async_create_file_by_raw(
        *,
        user_id: str,
        tenant_id: str,
        conversation_id: Optional[str],
        ...
        filename = f"{unique_name}{extension}"
        filepath = f"tools/{tenant_id}/{filename}"
        await async_storage.save(filepath, file_binary)
    ...
    @staticmethod
    async def async_create_file_by_url(
        user_id: str,
        tenant_id: str,
        conversation_id: str | None,
        file_url: str,
        ...
        filename = f"{unique_name}{extension}"
        filepath = f"tools/{tenant_id}/{filename}"
        await async_storage.save(filepath, blob)
    ...
    def get_file_generator_by_tool_file_id(tool_file_id: str):
        ...
        return stream, tool_file.mimetype


# init tool_file_parser
from agent_platform_core.file.tool_file_parser import tool_file_manager
