...
from agent_platform_core.app.app_config.common.sensitive_word_avoidance.manager import \
...
from agent_platform_core.app.app_config.easy_ui_based_app.dataset.manager import DatasetConfigManager
from agent_platform_core.app.app_config.easy_ui_based_app.model_config.async_manager import ModelConfigManagerAsync
from agent_platform_core.app.app_config.easy_ui_based_app.model_config.manager import ModelConfigManager
from agent_platform_core.app.app_config.easy_ui_based_app.prompt_template.manager import PromptTemplateConfigManager
from agent_platform_core.app.app_config.easy_ui_based_app.variables.manager import BasicVariablesConfigManager
from agent_platform_core.app.app_config.entities import EasyUIBasedAppConfig, EasyUIBasedAppModelConfigFrom
...
from agent_platform_core.models.db_model.asyncs.app_async import AppAsync
from agent_platform_core.models.db_model.model import App, AppModelConfig, Conversation
...
class AgentChatAppConfigAsync(BaseAppConfigManager):

    def __init__(self, async_manager_service: ModelConfigManagerAsync = Depends(ModelConfigManagerAsync), app_async: AppAsync = Depends(AppAsync)):
        self.app_async = app_async
        self.async_manager_service = async_manager_service
    ...
    async def get_app_config(self, app_model: App,
        ...
        if config_from != EasyUIBasedAppModelConfigFrom.ARGS:
            app_model_config_dict = await self.app_async.to_dict(app_model_config)
        ...
        app_config = AgentChatAppConfig(
            ...
            additional_features=self.convert_features(config_dict, app_mode)
        )
    ...
    async def config_validate(self, tenant_id: str, config: dict):
        """
        ...
        # model
        config, current_related_config_keys = await self.async_manager_service.validate_and_set_defaults(tenant_id,
                                                                                                         config)
        ...
        # agent_mode
        config, current_related_config_keys = self.validate_agent_mode_and_set_defaults(tenant_id, config)
        related_config_keys.extend(current_related_config_keys)
    ...
    def validate_agent_mode_and_set_defaults(self, tenant_id: str, config: dict) -> tuple[dict, list[str]]:
        """
