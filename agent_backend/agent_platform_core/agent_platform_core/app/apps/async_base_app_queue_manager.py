...
class AsyncAppQueueManager:
    def __init__(self, task_id: str, user_id: str, invoke_from: InvokeFrom) -> None:
        ...
        redis_client.setex(
            AsyncAppQueueManager._generate_task_belong_cache_key(self._task_id), 1800, f"{user_prefix}-{self._user_id}"
        )
    ...
    async def listen(self) -> AsyncGenerator:
        ...
        listen_timeout = agent_platform_config.APP_MAX_EXECUTION_TIME
        start_time = time.time()
        last_ping_time = 0
        while True:
            try:
                message = await asyncio.wait_for(self._q.get(), timeout=1)
                ...
                if elapsed_time >= listen_timeout or await self._is_stopped():
    ...
    async def publish_error(self, e, pub_from: PublishFrom) -> None:
        """
        Publish error
        :param e: error
        ...
        :return:
        """
        await self.publish(QueueErrorEvent(error=e), pub_from)
    ...
    async def publish(self, event: AppQueueEvent, pub_from: PublishFrom) -> None:
        ...
        await self._publish(event, pub_from)
    ...
    @classmethod
    async def set_stop_flag(cls, task_id: str, invoke_from: InvokeFrom, user_id: str) -> None:
        """
        Set task stop flag
        :return:
        """
        result = await async_redis_client.get(cls._generate_task_belong_cache_key(task_id))
        if result is None:
            return
        ...
        await async_redis_client.setex(stopped_cache_key, 600, 1)
    ...
    async def _is_stopped(self) -> bool:
        ...
        :return:
        """
        stopped_cache_key = AsyncAppQueueManager._generate_stopped_cache_key(self._task_id)
        result = await async_redis_client.get(stopped_cache_key)
