...
from agent_platform_core.models.db_model.model import App, Tag, TagBinding
...
class TagService:
    ...
    async def get_target_ids_by_tag_ids_async(self, tag_type: str, current_tenant_id: str, tag_ids: str) -> list:
        result = await self.session.execute(select(Tag).where(
            Tag.id.in_(tag_ids.split(",")),
            Tag.tenant_id == current_tenant_id,
            Tag.type == tag_type
        ))
        ...
        if not tags:
            return []
        tag_ids = [tag.id for tag in tags]
        tag_bindings_query = await self.session.execute(select(
            TagBinding.target_id
        ).where(
            TagBinding.tag_id.in_(tag_ids),
            TagBinding.tenant_id == current_tenant_id
        ))
        tag_bindings = tag_bindings_query.scalars().all()
        return tag_bindings
        # if not tag_bindings:
        #     return []
        # results = [tag_binding.target_id for tag_binding in tag_bindings]
