...
from agent_platform_core.models.db_model.dataset import Dataset, Document, DocumentSegment
...
class AsyncDatasetMultiRetrieverTool(AsyncDatasetRetrieverBaseTool):
    ...
    async def _async_run(self, query: str) -> str:
        ...
        rerank_model_instance = await model_manager.get_model_instance(
        ...
        all_documents = await rerank_runner.run(query, all_documents, self.score_threshold, self.top_k)

        for hit_callback in self.hit_callbacks:
            await hit_callback.on_tool_end(all_documents)
        ...
        if segments:
            ...
            if self.return_resource:
                ...
                for hit_callback in self.hit_callbacks:
                    await hit_callback.return_retriever_resource_info(context_list)

            return str("\n".join(document_context_list))

        raise RuntimeError("not segments found")

    async def _retriever(self, dataset_id: str, query: str, all_documents: list,
