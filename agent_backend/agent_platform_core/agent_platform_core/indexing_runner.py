...
from agent_platform_core.tools.utils.web_reader_tool import get_image_upload_file_ids

from agent_platform_common.configs import agent_platform_config
from agent_platform_core.entities.knowledge_entities import QAPreviewDetail, PreviewDetail, IndexingEstimate
...
from agent_platform_core.errors.error import ProviderTokenNotInitError
...
from agent_platform_core.model_runtime.entities.model_entities import ModelType, PriceType
from agent_platform_core.model_runtime.model_providers.__base.large_language_model import LargeLanguageModel
from agent_platform_core.model_runtime.model_providers.__base.text_embedding_model import TextEmbeddingModel
from agent_platform_core.models.db_model.dataset import Dataset, DatasetProcessRule, DocumentSegment
...
from agent_platform_core.rag.docstore.dataset_docstore import DatasetDocumentStore
from agent_platform_core.rag.extractor.entity.extract_setting import ExtractSetting
from agent_platform_core.rag.index_processor.index_processor_base import BaseIndexProcessor
from agent_platform_core.rag.index_processor.index_processor_factory import IndexProcessorFactory
...
from agent_platform_core.model_runtime.model_providers.__base.tokenizers.gpt2_tokenzier import GPT2Tokenizer
...
class IndexingRunner:
    ...
    def run(self, dataset_documents: list[DatasetDocument]):
        ...
        for dataset_document in dataset_documents:
            try:
                ...
                dataset_document.stopped_at = datetime.datetime.now().replace(tzinfo=None)
                db.session.commit()
    ...
    def indexing_estimate(self, tenant_id: str, extract_settings: list[ExtractSetting], tmp_processing_rule: dict,
                          ...
                          indexing_technique: str = 'economy') -> IndexingEstimate:
        """
        ...
        if features.billing.enabled:
            ...
            batch_upload_limit = agent_platform_config.BATCH_UPLOAD_LIMIT
                ...
                if len(preview_texts) < 10:
                    if doc_form and doc_form == "qa_model":
                        preview_detail = QAPreviewDetail(
                            question=document.page_content, answer=document.metadata.get("answer") or "", num_tokens=GPT2Tokenizer.get_num_tokens(document.page_content)
                        )
                        preview_texts.append(preview_detail)
                    else:
                        preview_detail = PreviewDetail(content=document.page_content, num_tokens=GPT2Tokenizer.get_num_tokens(document.page_content))  # type: ignore
                        if document.children:
                            preview_detail.child_chunks = [child.page_content for child in document.children]  # type: ignore
                        preview_texts.append(preview_detail)
                image_upload_file_ids = get_image_upload_file_ids(document.page_content)
                for upload_file_id in image_upload_file_ids:
                    image_file = UploadFile.query.filter_by(id=upload_file_id).first()
                    try:
                        if image_file:
    ...
    def _extract(self, index_processor: BaseIndexProcessor, dataset_document: DatasetDocument, process_rule: dict) \
            -> list[Document]:
        # load file
        if dataset_document.data_source_type not in ["upload_file"]:
            return []
        ...
        self._update_document_index_status(
            ...
            extra_update_params={
                ...
                DatasetDocument.parsing_completed_at: datetime.datetime.now().replace(tzinfo=None)
            }
        )
    ...
    def _step_split(self, text_docs: list[Document], splitter: TextSplitter,
        ...
        self._update_segments_by_document(
            ...
            update_params={
                ...
                DocumentSegment.indexing_at: datetime.datetime.now().replace(tzinfo=None)
            }
        )
    ...
    def _load(self, index_processor: BaseIndexProcessor, dataset: Dataset,
        ...
        self._update_document_index_status(
            ...
            extra_update_params={
                ...
                DatasetDocument.completed_at: datetime.datetime.now().replace(tzinfo=None),
        ...
        with flask_app.app_context():
            ...
            if dataset.indexing_technique != 'high_quality':
                ...
                db.session.query(DocumentSegment).filter(
                    ...
                    DocumentSegment.completed_at: datetime.datetime.now().replace(tzinfo=None)
                })
