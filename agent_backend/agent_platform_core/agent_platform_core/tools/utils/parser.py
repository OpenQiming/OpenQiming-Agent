...
from agent_platform_core.tools.entities.tool_entities import ApiProviderSchemaType, ToolParameter
from agent_platform_core.tools.errors import ToolApiSchemaError, ToolNotSupportedError, ToolProviderNotFoundError
    ...
    def parse_openapi_to_tool_bundle(
        ...
        for path, path_item in openapi["paths"].items():
            ...
            for method in methods:
                if method.upper() in path_item:
                    interfaces.append(
                        {
                            "path": path,
                            "method": method,
                            "operation": path_item[method.upper()],
                        }
                    )
