...
from agent_platform_basic.exceptions.services.dataset import DatasetNameDuplicateError
from agent_platform_basic.exceptions.services.document import DocumentIndexingError
from agent_platform_basic.exceptions.services.file import FileNotExistsError
...
from agent_platform_core.errors.error import LLMBadRequestError, ProviderTokenNotInitError
...
from agent_platform_core.models.db_model.dataset import (
    AppDatasetJoin,
    Dataset,
...
from agent_platform_core.rag.datasource.keyword.keyword_factory import Keyword
from agent_platform_core.rag.extractor.entity.extract_setting import RetrievalModel
from agent_platform_core.rag.models.document import Document as RAGDocument
from agent_platform_core.rag.retrieval.retrival_methods import RetrievalMethod
from agent_platform_service.events.dataset_event import dataset_was_deleted
from agent_platform_service.events.document_event import document_was_deleted
from agent_platform_service.services.feature_service import FeatureModel, FeatureService
from agent_platform_service.services.tag_service import TagService
from agent_platform_service.services.vector_service import VectorService
from agent_platform_service.tasks.batch_clean_document_task import batch_clean_document_task
from agent_platform_service.tasks.deal_dataset_vector_index_task import deal_dataset_vector_index_task
from agent_platform_service.tasks.delete_segment_from_index_task import delete_segment_from_index_task
from agent_platform_service.tasks.disable_segment_from_index_task import disable_segment_from_index_task
from agent_platform_service.tasks.document_indexing_task import document_indexing_task
from agent_platform_service.tasks.document_indexing_update_task import document_indexing_update_task
from agent_platform_service.tasks.duplicate_document_indexing_task import duplicate_document_indexing_task
from agent_platform_service.tasks.recover_document_indexing_task import recover_document_indexing_task
from agent_platform_service.tasks.retry_document_indexing_task import retry_document_indexing_task
from agent_platform_service.tasks.sync_website_document_indexing_task import sync_website_document_indexing_task
...
class DatasetService:
    ...
    def get_datasets(page, per_page, provider="vendor", tenant_id=None, user=None, search=None, tag_ids=None):
        ...
        query = Dataset.query.filter(
            db.and_(Dataset.provider == provider, Dataset.tenant_id == tenant_id)) \
    ...
    def update_dataset(dataset_id, data, user):
        ...
        def update_filtered_data_when_high_quality():
        ...
        if dataset.indexing_technique != data['indexing_technique']:
            ...
            if data['indexing_technique'] == 'economy':
                ...
                action = 'add'
                update_filtered_data_when_high_quality()
        else:
            if StringUtils.not_equal(data['embedding_model_provider'], dataset.embedding_model_provider) or \
                    StringUtils.not_equal(data['embedding_model'], dataset.embedding_model):
                action = 'update'
                update_filtered_data_when_high_quality()

        filtered_data['updated_by'] = user.id
        filtered_data['updated_at'] = datetime.datetime.now().replace(tzinfo=None)
...
class DocumentService:
    ...
    def pause_document(document):
        ...
        document.paused_at = datetime.datetime.now().replace(tzinfo=None)
            ...
            if not dataset_process_rule:
                ...
                if process_rule["mode"] in ("custom", "hierarchical"):
                    dataset_process_rule = DatasetProcessRule(
                        ...
                        rules=json.dumps(process_rule["rules"], ensure_ascii=False),
    ...
    def update_document_with_dataset_id(dataset: Dataset, document_data: dict,
        ...
        document.updated_at = datetime.datetime.now().replace(tzinfo=None)
        document.created_from = created_from
        document.doc_form = document_data['doc_form']
        db.session.add(document)
        db.session.commit()
    ...
    def create_segment(cls, args: dict, document: Document, dataset: Dataset):
        ...
        with redis_client.lock(lock_name, timeout=600):
            ...
            segment_document = DocumentSegment(
                ...
                tokens=tokens,
                status='completed',
                indexing_at=datetime.datetime.now().replace(tzinfo=None),
                completed_at=datetime.datetime.now().replace(tzinfo=None),
            ...
            try:
                ...
                segment_document.disabled_at = datetime.datetime.now().replace(tzinfo=None)
    ...
    def update_segment(cls, args: dict, segment: DocumentSegment, document: Document, dataset: Dataset):
        ...
        if 'enabled' in args and args['enabled'] is not None:
            ...
            if segment.enabled != action:
                if not action:
                    ...
                    segment.disabled_at = datetime.datetime.now().replace(tzinfo=None)
        ...
        try:
            ...
            if segment.content == content:
                ...
                segment.indexing_at = datetime.datetime.now().replace(tzinfo=None)
                segment.completed_at = datetime.datetime.now().replace(tzinfo=None)
                segment.updated_by = current_user.id
                segment.updated_at = datetime.datetime.now().replace(tzinfo=None)
                segment.enabled = True
                segment.disabled_at = None
                segment.disabled_by = None
                if document.doc_form == 'qa_model':
