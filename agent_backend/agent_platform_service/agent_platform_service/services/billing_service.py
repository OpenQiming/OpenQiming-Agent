...
from agent_platform_basic.libs.helper import RateLimiter
...
class BillingService:
    ...
    class EducationIdentity:
        ...
        def verify(cls, account_id: str, account_email: str):
            if cls.verification_rate_limit.is_rate_limited(account_email):
                from agent_platform_basic.exceptions.controllers.console_exceptions import EducationVerifyLimitError
        ...
        def activate(cls, account: Account, token: str, institution: str, role: str):
            if cls.activation_rate_limit.is_rate_limited(account.email):
                from agent_platform_basic.exceptions.controllers.console_exceptions import EducationActivateLimitError
    ...
    def get_compliance_download_link(
        ...
        if cls.compliance_download_rate_limiter.is_rate_limited(limiter_key):
            from agent_platform_basic.exceptions.controllers.console_exceptions import CompilanceRateLimitError
