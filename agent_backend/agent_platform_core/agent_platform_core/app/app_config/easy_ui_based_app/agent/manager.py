...
from agent_platform_core.agent.entities import AgentEntity, AgentPromptEntity, AgentToolEntity
from agent_platform_core.agent.prompt.template import REACT_PROMPT_TEMPLATES
from agent_platform_core.agent.prompt.llama_template import LLAMA_PROMPT_TEMPLATES
...
class AgentConfigManager:
    ...
    def convert(cls, config: dict) -> Optional[AgentEntity]:
        ...
        if "agent_mode" in config and config["agent_mode"] and "enabled" in config["agent_mode"]:
            ...
            elif agent_strategy == 'qiming_call':
                strategy = AgentEntity.Strategy.QIMING_CALLING
            else:
            ...
            if "strategy" in config["agent_mode"] and config["agent_mode"]["strategy"] not in {
                ...
                if model_mode == "completion":
                    ...
                    if agent_strategy == 'qiming_call':
                        agent_prompt_entity = AgentPromptEntity(
                            first_prompt=agent_prompt.get(
                                "first_prompt", LLAMA_PROMPT_TEMPLATES["english"]["chat"]["prompt"]
                            ),
                            next_iteration=agent_prompt.get(
                                "next_iteration", LLAMA_PROMPT_TEMPLATES["english"]["chat"]["agent_scratchpad"]
                            ),
                        )
                    else:
                        agent_prompt_entity = AgentPromptEntity(
