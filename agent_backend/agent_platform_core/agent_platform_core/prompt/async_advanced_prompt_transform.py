...
from agent_platform_core.file import file_manager
from agent_platform_core.file.models import File
from agent_platform_core.helper.code_executor.jinja2.jinja2_formatter import Jinja2Formatter
...
from agent_platform_core.model_runtime.entities.message_entities import ImagePromptMessageContent
...
from agent_platform_core.prompt.async_prompt_transform import AsyncPromptTransform
...
class AsyncAdvancedPromptTransform(AsyncPromptTransform):
    """
    ...
    async def get_prompt(
        ...
        *,
        ...
        memory: Optional[AsyncTokenBufferMemory],
        model_config: ModelConfigWithCredentialsEntity,
    ) -> list[PromptMessage]:
        prompt_messages = []

        if isinstance(prompt_template, CompletionModelPromptTemplate):
            prompt_messages = await self._get_completion_model_prompt_messages(
            ...
            prompt_messages = await self._get_chat_model_prompt_messages(
                prompt_template=prompt_template,
                inputs=inputs,
                query=query,
                files=files,
    ...
    async def _get_completion_model_prompt_messages(
        self,
        ...
        if prompt_template.edition_type == "basic" or not prompt_template.edition_type:
            ...
            if memory and memory_config:
                ...
                prompt_inputs = await self._set_histories_variable(
                    memory=memory,
                    memory_config=memory_config,
    ...
    async def _get_chat_model_prompt_messages(
        self,
        ...
        if memory and memory_config:
            prompt_messages = await self._append_chat_histories(memory, memory_config, prompt_messages, model_config)
    ...
    async def _set_histories_variable(
        self,
        ...
        if "#histories#" in parser.variable_keys:
            if memory:
                ...
                rest_tokens = await self._calculate_rest_token([tmp_human_message], model_config)

                histories = await self._get_history_messages_from_memory(
                    memory=memory,
                    memory_config=memory_config,
