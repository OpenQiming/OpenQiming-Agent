...
class Tool(BaseModel, ABC):
    ...
    async def async_invoke(self, user_id: str, tool_parameters: Mapping[str, Any]) -> list[ToolInvokeMessage]:
        # update tool_parameters
        # TODO: Fix type error.
        if self.runtime.runtime_parameters:
            tool_parameters.update(self.runtime.runtime_parameters)
        ...
        # 记录外层app的tenant_id, 内层工具会重新设置tenant_id, 记录用于内层工具执行完后恢复
        try:
            outer_app_tenant_id = contexts.tenant_id.get()
        except LookupError:
            # 单节点运行
            outer_app_tenant_id = None
        ...
        # 恢复外层app的tenant_id
        contexts.tenant_id.set(outer_app_tenant_id)
