...
from agent_platform_core.app.entities.app_invoke_entities import AdvancedChatAppGenerateEntity, InvokeFrom, WorkflowAppGenerateEntity
from agent_platform_core.app.entities.queue_entities import (
    ...
    QueueParallelBranchRunSucceededEvent, QueueNodeRetryEvent,
)
from agent_platform_core.app.entities.task_entities import (
    ...
    WorkflowTaskState, NodeRetryStreamResponse,
)
...
from agent_platform_core.ops.entities.trace_entity import TraceTaskName
from agent_platform_core.ops.ops_trace_manager import TraceQueueManager, TraceTask
from agent_platform_core.tools.async_tool_manager import AsyncToolManager
from agent_platform_core.workflow.entities.node_entities import NodeRunMetadataKey
from agent_platform_core.workflow.entities.workflow_execution import WorkflowExecutionStatus
...
from agent_platform_core.workflow.nodes.tool.entities import ToolNodeData
...
from agent_platform_core.models.enum_model.enums import CreatedByRole, WorkflowRunTriggeredFrom
from agent_platform_core.models.db_model.model import EndUser
from agent_platform_core.models.db_model.workflow import (
    Workflow,
    WorkflowNodeExecution,
    WorkflowRun,
)
from agent_platform_core.models.enum_model.workflow import (
    WorkflowNodeExecutionStatus,
    WorkflowNodeExecutionTriggeredFrom,
    WorkflowRunStatus,
)
from agent_platform_service.fields.model_async.end_user_async import EndUserAsync
from agent_platform_service.fields.model_async.workflow_run_async import WorkflowRunAsync


class AsyncWorkflowCycleManage:
    ...
    async def _handle_workflow_run_start(self) -> WorkflowRun:
        async with async_db.AsyncSessionLocal() as session:
            max_sequence = await session.execute(select(func.max(WorkflowRun.sequence_number)).filter(
                WorkflowRun.tenant_id == self._workflow.tenant_id).filter(WorkflowRun.app_id == self._workflow.app_id))
            max_sequence = max_sequence.scalar() or 0
        ...
        workflow_run.inputs = json.dumps(inputs, ensure_ascii=False)
        ...
        workflow_run.created_at = datetime.now().replace(tzinfo=None)
    ...
    async def _handle_workflow_run_success(
        self,
        workflow_run: WorkflowRun,
        start_at: float,
        total_tokens: int,
        ...
        """
        async with async_db.AsyncSessionLocal() as session:
            workflow_run = await self._refetch_workflow_run(session, workflow_run.id)

            outputs = WorkflowEntry.handle_special_values(outputs)

            workflow_run.status = WorkflowRunStatus.SUCCEEDED.value
            workflow_run.outputs = json.dumps(outputs or {}, ensure_ascii=False)
            workflow_run.elapsed_time = time.perf_counter() - start_at
            workflow_run.total_tokens = total_tokens
            workflow_run.total_steps = total_steps
            workflow_run.finished_at = datetime.now().replace(tzinfo=None)
    ...
    async def _handle_workflow_run_failed(
        self,
        workflow_run: WorkflowRun,
        start_at: float,
        total_tokens: int,
        ...
        async with async_db.AsyncSessionLocal() as session:
            ...
            running_workflow_node_executions_scalar = await session.execute(
                select(WorkflowNodeExecution).filter(
            ...
            running_workflow_node_executions = running_workflow_node_executions_scalar.scalars().all()

            for workflow_node_execution in running_workflow_node_executions:
                workflow_node_execution.status = WorkflowNodeExecutionStatus.FAILED.value
                workflow_node_execution.error = error
                workflow_node_execution.finished_at = datetime.now().replace(tzinfo=None)
    ...
    async def _handle_node_execution_start(
        ...
        async with async_db.AsyncSessionLocal() as session:
            ...
            workflow_node_execution.created_at = datetime.now().replace(tzinfo=None)
    ...
    async def _handle_workflow_node_execution_success(self, event: QueueNodeSucceededEvent) -> WorkflowNodeExecution:
        """
        ...
        execution_metadata = (
            json.dumps(jsonable_encoder(event.execution_metadata), ensure_ascii=False) if event.execution_metadata else None
        )
        ...
        workflow_node_execution.inputs = json.dumps(inputs, ensure_ascii=False) if inputs else None
        workflow_node_execution.process_data = json.dumps(process_data, ensure_ascii=False) if process_data else None
        workflow_node_execution.outputs = json.dumps(outputs, ensure_ascii=False) if outputs else None
        workflow_node_execution.execution_metadata = execution_metadata
        workflow_node_execution.finished_at = finished_at
        workflow_node_execution.elapsed_time = elapsed_time

        async with async_db.AsyncSessionLocal() as session:
            await session.merge(workflow_node_execution)
            await session.commit()

        # workflow_node_execution.update_from_mapping(
        #     inputs=inputs, process_data=process_data, outputs=outputs, metadata=execution_metadata
        # )
    ...
    async def _handle_workflow_node_execution_retried(self, workflow_run: WorkflowRun, event: QueueNodeRetryEvent) -> WorkflowNodeExecution:
        ...
        workflow_node_execution.update_from_mapping(inputs=inputs, outputs=outputs, metadata=merged_metadata)
    ...
    async def _handle_workflow_run_partial_success(
            self,
            *,
            workflow_run: WorkflowRun,
            total_tokens: int,
        ...
        async with async_db.AsyncSessionLocal() as session:
            ...
            workflow_run.status = WorkflowExecutionStatus.PARTIAL_SUCCEEDED
            workflow_run.outputs = outputs or {}
            workflow_run.total_tokens = total_tokens
            workflow_run.total_steps = total_steps
            ...
            await session.merge(workflow_run)
            await session.commit()

        return workflow_run

    async def _handle_workflow_node_execution_failed(
    ...
    async def _workflow_finish_to_stream_response(
        self, task_id: str, workflow_run: WorkflowRun
    ) -> WorkflowFinishStreamResponse:
        """
        ...
        if workflow_run.created_by_role == CreatedByRole.ACCOUNT.value:
            created_by_account = await WorkflowRunAsync.created_by_account(created_by=workflow_run.created_by,
                                                                           created_by_role=workflow_run.created_by_role)
            ...
            created_by_end_user = await EndUserAsync.created_by_end_user(created_by=workflow_run.created_by,
                                                                         created_by_role=workflow_run.created_by_role)
    ...
    async def _workflow_node_start_to_stream_response(
        ...
        if event.node_type == NodeType.TOOL:
            ...
            response.data.extras["icon"] = await AsyncToolManager.get_tool_icon(
    ...
    def _workflow_iteration_completed_to_stream_response(
        ...
        return IterationNodeCompletedStreamResponse(
            ...
            data=IterationNodeCompletedStreamResponse.Data(
                ...
                error=None,
                elapsed_time=(datetime.now().replace(tzinfo=None) - event.start_at).total_seconds(),
    ...
    async def _refetch_workflow_run(self, session: AsyncSession, workflow_run_id: str) -> WorkflowRun:
        """
        Refetch workflow run
        :param workflow_run_id: workflow run id
        :return:
        """
        workflow_run = await session.execute(select(WorkflowRun).filter(WorkflowRun.id == workflow_run_id))
