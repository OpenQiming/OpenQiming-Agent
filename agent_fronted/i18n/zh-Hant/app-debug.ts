const translation = {
  ...
  feature: {
    ...
    tools: {
      title: '插件',
      tips: '插件提供了一個標準的 API 呼叫方式，將使用者輸入或變數作為 API 的請求引數，用於查詢外部資料作為上下文。',
      toolsInUse: '{{count}} 插件使用中',
      modal: {
        title: '插件',
        toolType: {
          title: '插件型別',
  ...
  assistantType: {
    ...
    agentAssistant: {
      name: '智慧助手',
      description: '構建一個智慧助手，他可以自主選擇插件完成你設定的任務',
    },
  },
  agent: {
    ...
    tools: {
      name: '插件',
      description: '使用插件可以擴充套件代理的能力，比如搜尋網際網路或科學計算',
      enabled: '啟用',
    },
  },
}
