...
from agent_platform_core.models.db_model.dataset import Dataset, DatasetProcessRule, Document, DocumentSegment
...
from agent_platform_service.fields.document_fields import (
...
from agent_platform_service.tasks.add_document_to_index_task import add_document_to_index_task
from agent_platform_service.tasks.remove_document_from_index_task import remove_document_from_index_task
...
class DocumentResource(Resource):
    ...
    def get_batch_documents(self, dataset_id: str, batch: str, tenant_id: str) -> list[Document]:
        dataset = DatasetService.get_dataset(dataset_id)
        if not dataset:
            raise NotFound('Dataset not found.')
...
class DocumentBatchIndexingStatusApi(DocumentResource):
    ...
    @account_initialization_required
    def get(self, dataset_id, batch):
        tenant_id = request.args.get('tenant_id', default="", type=str)
        dataset_id = str(dataset_id)
        batch = str(batch)
        documents = self.get_batch_documents(dataset_id, batch, tenant_id)
        documents_status = []
        for document in documents:
            completed_segments = DocumentSegment.query.filter(DocumentSegment.completed_at.isnot(None),
                                                              DocumentSegment.document_id == str(document.id),
...
class DocumentProcessingApi(DocumentResource):
    ...
    def patch(self, dataset_id, document_id, action):
        ...
        if action == "pause":
            ...
            document.paused_at = datetime.now().replace(tzinfo=None)
...
class DocumentMetadataApi(DocumentResource):
    ...
    def put(self, dataset_id, document_id):
        ...
        document.updated_at = datetime.now().replace(tzinfo=None)
        db.session.commit()
...
class DocumentStatusApi(DocumentResource):
    ...
    def patch(self, dataset_id, document_id, action):
        ...
        if action == "enable":
            ...
            document.disabled_at = datetime.now().replace(tzinfo=None)
            ...
            document.archived_at = datetime.now().replace(tzinfo=None)
...
api.add_resource(DocumentStatusApi,
                 '/datasets/<uuid:dataset_id>/documents/<uuid:document_id>/status/<string:action>/batch')
