...
from agent_platform_core.models.enum_model.workflow import WorkflowRunTriggeredFrom
...
class WorkflowDailyRunsStatistic(Resource):
    ...
    def get(self, app_model):
        ...
        SELECT date(DATE_TRUNC('day', created_at AT TIME ZONE 'UTC' AT TIME ZONE :tz )) AS date, count(id) AS runs
            FROM workflow_runs 
            WHERE app_id = :app_id 
                AND triggered_from in ('debugging', 'app-run')
        '''
...
class WorkflowAverageAppInteractionStatistic(Resource):
    ...
    def get(self, app_model):
        ...
        sql_query = """
            ...
            FROM
                ...
                WHERE c.app_id = :app_id
                    AND c.triggered_from in ('debugging', 'app-run')
                    {{start}}
                    {{end}}
        ...
        response_data = []
        print("sql_query:::", sql_query, arg_dict)
        with db.engine.begin() as conn:
            rs = conn.execute(db.text(sql_query), arg_dict)
            for i in rs:
                response_data.append({
...
api.add_resource(WorkflowAverageAppInteractionStatistic,
