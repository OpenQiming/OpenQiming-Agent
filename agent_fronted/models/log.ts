...
export type ConversationMoveAppId = {
  mode?: string,
  to_tenant_id?: string | number
}

export type ConversationMoveAppIds = {
  name?: string,
  tenant_id?: string
}
...
export type ChatConversationFullDetailResponse =
  Omit<CompletionConversationGeneralDetail, 'message' | 'model_config'>
  & {
  message_count: number
  model_config: {
    provider: string
...
export type AdminAppliedType = AdminApplyingType & {
  reviewer: string
  reviewer_id: string
  reviewed_at: string
  created_at: string
...
export type ResultApplyingList = {
  page: number
  limit: number
  total: number
  has_more: boolean
  data: AdminApplyingType[]
}
export type ResultApplyedList = {
  page: number
  limit: number
  total: number
  has_more: boolean
  data: ResultReviewList[]
}

export type createTenantType = {
  name: string | null,
  description: string | null,
  accounts: Array<createTenantTypeExtend>,
}

export type createTenantTypeExtend = {
  id: string,
  role: string
}
