...
from agent_platform_basic.extensions.ext_mail import mail
    ...
    try:
        ...
        if language == 'zh-Hans':
            html_content = render_template('invite_member_mail_template_zh-CN.html',
                                           ...
                                           url=url)
            mail.send(to=to, subject="立即加入 Agent Platform 工作空间", html=html_content)
        else:
            html_content = render_template('invite_member_mail_template_en-US.html',
                                           ...
                                           workspace_name=workspace_name,
                                           url=url)
            mail.send(to=to, subject="Join Agent Platform Workspace Now", html=html_content)
