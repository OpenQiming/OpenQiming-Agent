...
class ToolTransform:
    ...
    def repack_provider(provider: Union[dict, UserToolProvider]):
        ...
        if isinstance(provider, dict) and 'icon' in provider:
            provider['icon'] = ToolTransform.get_tool_provider_icon_url(
    ...
    def api_provider_to_controller(
        ...
        if db_provider is None or db_provider.credentials is None:
            raise ValueError("db_provider 或 db_provider.credentials 不能为 None")
    ...
    async def async_workflow_provider_to_controller(
            db_provider: WorkflowToolProvider
    ) -> WorkflowToolProviderController:
        """
        convert provider controller to provider
