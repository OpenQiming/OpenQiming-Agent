...
from typing import Optional, Union, cast, AsyncGenerator
...
from agent_platform_core.model_runtime.entities.llm_entities import LLMMode, LLMResult, LLMResultChunk, \
    LLMResultChunkDelta
...
from agent_platform_core.model_runtime.entities.model_entities import (
    AIModelEntity,
...
from agent_platform_core.model_runtime.model_providers.qwen_openai_api_compatible._common import (_Common_Qwen_OAI_API_Compat)
from agent_platform_core.model_runtime.utils import helper

logger = logging.getLogger(__name__)

class QwenOAIAPICompatLargeLanguageModel(_Common_Qwen_OAI_API_Compat, LargeLanguageModel):
    """
    ...
    async def _async_invoke(self, model: str, credentials: dict,
                            prompt_messages: list[PromptMessage], model_parameters: dict,
                            tools: Optional[list[PromptMessageTool]] = None, stop: Optional[list[str]] = None,
                            stream: bool = True, user: Optional[str] = None) \
    ...
    def get_customizable_model_schema(self, model: str, credentials: dict) -> AIModelEntity:
        ...
        elif function_calling_type in ['qiming_call']:
            features.append(ModelFeature.QIMING_CALL)
    ...
    def _generate(self, model: str, credentials: dict, prompt_messages: list[PromptMessage], model_parameters: dict,
        ...
        if model=="qwen72b" and endpoint_url=="http://134.188.136.40:9001/v1/":
            headers["Authorization"] = "Bearer sk-GOMSsm91pPIhhY5RD6135c442dDb46E2AdB7F76574CaF161"
    ...
    async def _async_generate(self, model: str, credentials: dict, prompt_messages: list[PromptMessage],
                              ...
                              tools: Optional[list[PromptMessageTool]] = None, stop: Optional[list[str]] = None,
                              stream: bool = True,
                              user: Optional[str] = None) -> Union[LLMResult, AsyncGenerator]:
        """
        Invoke llm completion model
        ...
        if model == "qwen3-32b" and endpoint_url=="http://10.254.47.219:31068/member1/qwen3-32b-api/v1/":
            headers["App-Key"] = "1alqE5kWbQIsBJHCT2Lt"
            headers['App-Sign'] = 'be8eefb29964ca12de9f417f2342090939b7e7bc'
            data = {
                'model': model,
                "stream": stream,
        ...
        logger.debug("==================================================")
        logger.debug(f"Sending request to llm, endpoint: {endpoint_url}")
        logger.debug(json.dumps(data, ensure_ascii=False))
        logger.debug(json.dumps(headers, ensure_ascii=False))
        logger.debug("==================================================")
        if stream:
            ...
            async with httpx.AsyncClient(timeout=300.0) as client:
                response = await client.post(endpoint_url, headers=headers, json=data, timeout=300)
                return self._handle_generate_response(model, credentials, response, prompt_messages)
    ...
    async def _async_handle_generate_stream_response(self, model: str, credentials: dict,
        ...
        finish_reason = 'Unknown'
        async with httpx.AsyncClient(timeout=300.0) as client:
            async with client.stream("POST", endpoint_url, headers=headers, json=data, timeout=300) as response:
                if response.encoding is None or response.encoding == 'ISO-8859-1':
                    response.encoding = 'utf-8'
    ...
    def _handle_generate_response(self, model: str, credentials: dict, response: requests.Response | httpx.Response,
                                  prompt_messages: list[PromptMessage]) -> LLMResult:
        def regex_response_content(response_content: str):
            tool_str_match = re.search(r'\{.*\}', response_content)
            if tool_str_match:
                return json.loads(tool_str_match.group(0))
            return None
        ...
        if completion_type is LLMMode.CHAT:
            ...
            if function_calling_type == 'tool_call':
                # instruct_json = regex_response_content(response_content)
                ...
                #         "type": "function",
                #         "function": {
                #             "name": instruct_json.get("name", ""),
                #             "arguments": json.dumps(instruct_json.get('parameters', ''))
                #         }
                #     }]
                #     response_content = ''
    ...
    def _convert_prompt_message_to_dict(self, message: PromptMessage, credentials: Optional[dict] = None) -> dict:
        ...
        if isinstance(message, UserPromptMessage):
            ...
            if message.tool_calls:
                ...
                for function_call in message.tool_calls:
                    message_dict["tool_calls"].append({"id": function_call.id, 'function': {
        ...
        # elif isinstance(message, IpythonPromptMessage):
        #     # for llama3.1
        #     message = cast(IpythonPromptMessage, message)
        #     message_dict = {
        #         "role": "ipython",
        #         "content": message.content
        #     }
