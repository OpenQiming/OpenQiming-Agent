...
from agent_platform_core.workflow.nodes.llm import LLMNode
...
from agent_platform_core.model_runtime.entities.llm_entities import LLMUsage
from agent_platform_core.model_runtime.entities.message_entities import PromptMessage, PromptMessageRole, \
    PromptMessageTool
from agent_platform_core.prompt.advanced_prompt_transform import AdvancedPromptTransform
from agent_platform_core.prompt.entities.advanced_prompt_entities import ChatModelMessage, CompletionModelPromptTemplate
from agent_platform_core.rag.retrieval.output_parser.react_output import ReactAction
from agent_platform_core.rag.retrieval.output_parser.structured_chat import StructuredChatOutputParser
...
class ReactMultiDatasetRouter:
    ...
    async def invoke(
        ...
        try:
            return await self._react_invoke(query=query, model_config=model_config,
                                      model_instance=model_instance,
                                      tools=dataset_tools, user_id=user_id, tenant_id=tenant_id)
        except Exception as e:
            return None

    async def _react_invoke(
            self,
            query: str,
            model_config: ModelConfigWithCredentialsEntity,
            model_instance: AsyncModelInstance,
        ...
        result_text, usage = await self._invoke_llm(
    ...
    async def _invoke_llm(self, completion_param: dict,
                    model_instance: AsyncModelInstance,
        ...
        """
            ...
            :return:
        """
        invoke_result = await model_instance.invoke_llm(
            prompt_messages=prompt_messages,
