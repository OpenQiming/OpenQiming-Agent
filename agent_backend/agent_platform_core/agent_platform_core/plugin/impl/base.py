...
from agent_platform_core.model_runtime.errors.invoke import (
...
from agent_platform_core.model_runtime.errors.validate import CredentialsValidateFailedError

from agent_platform_core.plugin.endpoint.exc import EndpointSetupFailedError
from agent_platform_core.plugin.entities.plugin_daemon import PluginDaemonBasicResponse, PluginDaemonError, PluginDaemonInnerError
from agent_platform_core.plugin.impl.exc import (
...
plugin_daemon_inner_api_baseurl = URL(str(agent_platform_config.PLUGIN_DAEMON_URL))
...
class BasePluginClient:
    def replace_uuid(self, path: str, new_uuid: str) -> str:
        """直接替换路径中的UUID字符串"""
        return re.sub(
            r'[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}',  # UUID正则
            new_uuid,
            path,
            flags=re.IGNORECASE
        )
    def _request(
        ...
        headers = headers or {}
        headers["X-Api-Key"] = agent_platform_config.PLUGIN_DAEMON_KEY
    ...
    def _request_with_plugin_daemon_response(
        ...
        """
        # 正则表达式过滤,替换tenant_id
        path = self.replace_uuid(path, agent_platform_config.GLOBAL_TENANT_ID)
        try:
            response = self._request(method, path, headers, data, params, files)
    ...
    def _request_with_plugin_daemon_response_stream(
        ...
        Make a stream request to the plugin daemon inner API and yield the response as a model.
        """
        # json.loads(data["data"]["credentials"]["servers_config"])
        ...
        for line in self._stream_request(method, path, params, headers, data, files):
            try:
                print("type:::", PluginDaemonBasicResponse[type])
