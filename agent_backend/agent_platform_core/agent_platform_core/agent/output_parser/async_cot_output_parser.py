...
from agent_platform_core.agent.entities import AgentScratchpadUnit
from agent_platform_core.model_runtime.entities.llm_entities import LLMResultChunk


class AsyncCotAgentOutputParser:
    @classmethod
    async def handle_react_stream_output(
        cls, llm_response: AsyncGenerator[LLMResultChunk, None], usage_dict: dict
    ) -> AsyncGenerator[Union[str, AgentScratchpadUnit.Action], None]:
        ...
        async for response in llm_response:
            ...
            while index < len(response):
                ...
                if code_block_delimiter_count == 3:
                    if in_code_block:
                        last_character = delta
                        for json_dict in extra_json_from_code_block(code_block_cache):
                            yield json_dict
                        code_block_cache = ""
