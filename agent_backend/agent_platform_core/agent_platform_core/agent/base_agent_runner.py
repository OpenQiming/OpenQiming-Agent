...
from agent_platform_core.agent.entities import AgentEntity, AgentToolEntity
...
from agent_platform_core.model_runtime.entities.model_entities import ModelFeature
...
# from agent_platform_core.tools.tool.dataset_retriever_tool import DatasetRetrieverTool
...
from agent_platform_core.models.db_model.model import Conversation, Message, MessageAgentThought, MessageFile
...
class BaseAgentRunner(AppRunner):
    def __init__(
        ...
        # init callback
        self.agent_callback = AgentPlatformAgentCallbackHandler()
        # init dataset tools
    ...
    def _init_prompt_tools(self) -> tuple[Mapping[str, Tool], Sequence[PromptMessageTool]]:
        ...
        for tool in self.app_config.agent.tools if self.app_config.agent else []:
            try:
                ...
                continue
            # save tool entity
            tool_instances[tool_entity.identity.name] = tool_entity
            # save prompt tool
    ...
    def update_db_variables(self, tool_variables: ToolRuntimeVariablePool, db_variables: ToolConversationVariables):
        ...
        db_variables.updated_at = datetime.now().replace(tzinfo=None)
    ...
    def organize_agent_history(self, prompt_messages: list[PromptMessage]) -> list[PromptMessage]:
        ...
        for messages in prompt_messages:
            if isinstance(messages, IpythonPromptMessage):
                return prompt_messages

        #TODO delete database query here
        result = []
