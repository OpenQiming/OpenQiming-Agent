...
from agent_platform_core.tools.provider.rag_tool_provider import RagProviderController
...
class AsyncToolTransform:
    ...
    def repack_provider(provider: Union[dict, UserToolProvider]):
        ...
        if isinstance(provider, dict) and 'icon' in provider:
            provider['icon'] = AsyncToolTransform.get_tool_provider_icon_url(
    ...
    @staticmethod
    async def api_provider_to_controller(
    ...
    @staticmethod
    async def workflow_provider_to_controller(
            db_provider: WorkflowToolProvider
    ) -> WorkflowToolProviderController:
        """
        convert provider controller to provider
        """
        return await WorkflowToolProviderController.async_from_db(db_provider)

    @staticmethod
    async def rag_provider_to_controller(
            db_provider: RagKbTableProvider,
            query: str = ""
    ):
        return await RagProviderController.async_from_db(db_provider, query)
    ...
    @staticmethod
    async def api_provider_to_user_provider(
