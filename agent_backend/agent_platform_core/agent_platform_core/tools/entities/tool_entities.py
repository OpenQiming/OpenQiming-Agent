...
from agent_platform_core.model_runtime.entities.provider_entities import ProviderConfig
from agent_platform_core.tools.entities.common_entities import I18nObject
from agent_platform_core.entities.provider_entities import ProviderConfig
from agent_platform_core.tools.entities.constants import TOOL_SELECTOR_MODEL_IDENTITY

from agent_platform_core.plugin.entities.parameters import (
    PluginParameterOption, PluginParameterType,
)
...
def safe_model_dump(value):
    ...
    elif isinstance(value, (str, bytes, dict)) or value is None:
        return value
    else:
        raise TypeError(f"Unsupported type for dumping: {type(value)}")
...
class ToolInvokeMessage(BaseModel):
    ...
    message: (
             # JsonMessage | TextMessage | BlobChunkMessage | BlobMessage | LogMessage | FileMessage | str | bytes | dict | None | VariableMessage
            str | bytes | dict | JsonMessage | TextMessage | BlobChunkMessage | BlobMessage | LogMessage | FileMessage | None | VariableMessage
    ) = None
